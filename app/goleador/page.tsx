  ...
  useEffect(() => {
    async function fetchGoleadores() {
      const { data, error } = await supabase
        .from('jugadores')
        .select(`
          id, nombre, numero_camisola, goles, 
          equipos(nombre), 
          categorias(nombre)
        `)
      ...
      if (!error && data) setGoleadores(data)
    }
    fetchGoleadores()
  }, [])
          ...
          .map((jugador) => (
            ...
            >
              <div className="flex flex-col items-center pr-4 border-r border-white/10">
                ...
                <span className="font-black text-2xl text-[#34D399] mt-1">{jugador.numero_camisola}</span>
              </div>
              
              <div className="flex-1 pl-4">
                <div className="flex justify-between items-start">
                    <h3 className="font-bold text-lg text-white">{jugador.nombre}</h3>
              ...
              <div className="ml-4 flex flex-col items-center border-l border-white/10 pl-4">
                ...
                <span className="text-[8px] text-[#34D399] uppercase tracking-widest font-bold mt-1 block mb-2">Goles</span>
                
                {/* Botón con el evento onClick que llama al confeti */}
                <button 
