...
export default function Home() {
  ...
  async function fetchPartidos() {
    // Consulta corregida: Se incluyen relaciones con jugadores y equipos dentro de goles
    ...
    if (data) {
      data.forEach((p) => {
        ...
        if (anterior && (p.goles_ep1 > anterior.goles_ep1 || p.goles_ep2 > anterior.goles_ep2)) {
          const ultimoGol = p.goles && p.goles.length > 0 ? p.goles[p.goles.length - 1] : null
          
          setMostrarGol(true)
          if (ultimoGol) {
            // CORRECCIÓN: Accedemos al elemento [0] porque las relaciones son arrays
            const jugador = ultimoGol.jugadores && Array.isArray(ultimoGol.jugadores) ? ultimoGol.jugadores[0] : ultimoGol.jugadores
            const equipo = ultimoGol.equipos && Array.isArray(ultimoGol.equipos) ? ultimoGol.equipos[0] : ultimoGol.equipos
          ...
          setTimeout(() => setGolInfo((prev: any) => ({ ...prev, [p.id]: null })), 15000)
        }
    ...
    <main className="min-h-screen bg-[#0B1120] text-white p-4 font-sans pb-28">
      ...
      <div className="space-y-3">
        {partidos.filter((p) => categoriaActiva === 'Todos' || p.categorias?.nombre === categoriaActiva).map((p) => (
          <div key={p.id} className={`p-3 rounded-xl border transition-all duration-500 ${golInfo[p.id] ? 'bg-green-900/20 border-green-500/50' : 'bg-[#111827] border-slate-800'}`}>
            ...
            <div className="flex justify-between items-center mb-1">
              <span className="text-[9px] uppercase text-zinc-500 font-bold tracking-wider">{p.categorias?.nombre}</span>
              <button onClick={() => toggleLike(p.id, p.likes)} disabled={likesLocales.includes(p.id)} className={`flex flex-col items-center gap-0 ${likesLocales.includes(p.id) ? 'text-[#34D399]' : 'text-cyan-400'}`}>
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="w-[40%] text-right font-bold text-sm truncate">{p.equipo1?.nombre}</div>
              ...
              <div className="w-[40%] text-left font-bold text-sm truncate">{p.equipo2?.nombre}</div>
            </div>
