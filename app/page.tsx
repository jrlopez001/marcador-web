    ...
    <main className="min-h-screen bg-[#0B1120] text-white p-4 font-sans pb-28">
      ...
      <div className="space-y-3">
        {partidos.filter((p) => categoriaActiva === 'Todos' || p.categorias?.nombre === categoriaActiva).map((p) => (
          <div key={p.id} className={`p-3 rounded-xl border transition-all duration-500 ${golEquipo[p.id] ? 'bg-green-900/40 border-green-500 shadow-[0_0_20px_rgba(34,197,94,0.3)]' : 'bg-[#111827] border-slate-800'}`}>
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="w-[40%] text-right font-bold text-sm truncate">
                {p.equipo1?.nombre}
                {golEquipo[p.id] === 'equipo1' && <span className="ml-1 text-[10px]">⚽</span>}
              </div>
              ...
              <div className="w-[40%] text-left font-bold text-sm truncate">
                {golEquipo[p.id] === 'equipo2' && <span className="mr-1 text-[10px]">⚽</span>}
                {p.equipo2?.nombre}
              </div>
            </div>
